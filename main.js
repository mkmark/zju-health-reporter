...
const LOGIN_INTL_URL =
  "https://login.microsoftonline.com/292620c5-ae10-4553-accf-68ec80325008/oauth2/v2.0/authorize?client_id=b85e9cdb-5584-4536-9108-5136aa1db133&response_type=code&redirect_uri=https%3a%2f%2fzjuam.zju.edu.cn%2fcas%2flogin%3fclient_name%3dAdfsClient&scope=https%3A%2F%2Fgraph.microsoft.com%2FUser.read";
...
var argv = require("yargs/yargs")(process.argv.slice(2))
  ...
  .example(
    "node $0 -u username -p password -n 30.000000 -e 120.0000000 --now --log"
  )
...
var log_file = fs.createWriteStream(
  __dirname + "/log/" + argv.username + ".log",
  { flags: "a" }
);
...
async function fill_vc(browser, page) {
  ...
  if (!IS_VC_RECOGNIZED) {
    console.log(
      "verifyCode error for too many times. Something is wrong. Exiting."
    );
    await browser.close();
    process.exit();
  }
...
async function try_submit(browser, page) {
  ...
  await page.click("div.list-box > div.footers > a"),
    argv.log &&
      (await page.screenshot({ path: "screenshot/submit_clicked.png" }));

  /// confirm
  await page.click("div.wapcf-btn.wapcf-btn-ok").catch(async (error) => {
  ...
  argv.log &&
    (await page.screenshot({ path: "screenshot/wapcf-btn-ok_clicked.png" }));
...
async function submit(browser, page) {
  ...
  if (!IS_SUCCESSFUL) {
    console.log(
      "Submission error for too many times. Something is wrong. Exiting."
    );
  }
}
